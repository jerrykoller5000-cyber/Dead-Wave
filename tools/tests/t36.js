(async () => {
  ...
  // a game key typed into the box does nothing to the game
  ...
  await wait(1500);
  ok(T.session.name === 'Jerry' && !document.getElementById('modeSelect').classList.contains('show'), 'Enter in the box starts the game as ' + T.session.name);
  let pats = 0, chis = 0, taps = 0; const A = T.AudioSys;
  const oP = A.gravePat, oC = A.chisel, oT = A.graveAmbience; A.gravePat = (...a) => { pats++; return oP(...a); }; A.chisel = () => { chis++; return oC(); }; A.graveAmbience = (...a) => { taps++; return oT(...a); };
  ...
  // blade meets the mound at the bottom of each stroke (posed exactly at the impact)
  const minRel = [], lat = [];
  for (let k = 0; k < 2; k++) {
    const d = c.diggers[k];
    ...
    minRel.push(tip.y - (g.y + T.moundHeight(tip.x - g.x, tip.z - g.z))); lat.push(Math.abs(tip.x - g.x));
  }
  ...
  // play again: name remembered, then he rises
  ...
  inp.value = 'Jerry II';
  document.getElementById('modeHunt').click(); await wait(800);
