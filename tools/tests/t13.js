(async () => {
  ...
  T.placeBuildAt('wall', rx + 1, rz, 1); T.placeBuildAt('floor', rx + 1, rz); T.placeBuildAt('floor', rx + 1, rz); // ground, then the deck above
  ...
  // #3: cutaway
  const roofCell = [gx, gz];
  for (let q = 0; q < 4; q++) T.placeBuildAt('wall', gx, gz + 1, q);
  p.set(T.gridCentre(gx), T.sampleHeight(T.gridCentre(gx), T.gridCentre(gz + 1)), T.gridCentre(gz + 1)); await wait(150);
  // player is inside; put a roof (need to be upstairs... place directly at lv1 via drag? use opts: stand on deck temporarily)
  p.set(T.gridCentre(rx + 1), deck.deck.deckY, T.gridCentre(rz)); await wait(100);
  const roof = T.placeBuildAt('floor', gx, gz + 1);
  const tur = T.placeBuildAt('light', gx, gz + 1, 0, { lv: 1 });
  p.set(T.gridCentre(gx), T.sampleHeight(T.gridCentre(gx), T.gridCentre(gz + 1)), T.gridCentre(gz + 1)); await wait(300);
  ...
  ok(roof.level === 1 && op < 0.5 && tur && tur.mesh.visible && top >= 0.5 && top < 0.9, 'roof over the marine goes see-through, turret up there stays mostly solid (roof ' + op + ', turret ' + top + ')');
  p.set(T.gridCentre(gx) + 8, T.sampleHeight(T.gridCentre(gx) + 8, T.gridCentre(gz + 1)), T.gridCentre(gz + 1)); await wait(300);
  op = 1; roof.mesh.traverse(o => { if (o.isMesh) op = Math.min(op, o.material.opacity); });
